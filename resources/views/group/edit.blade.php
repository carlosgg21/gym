            ...
            <div class="col-sm-6">
                <ol class="breadcrumb float-sm-left">
                    ...
                    <li class="breadcrumb-item"><a class="link-orange" href="{{ route('groups.index') }}"> <i
                                class="fa fa-list-alt" aria-hidden="true"></i> Listado de Grupos</a></li>
                    ...
                    <form method="POST" action="{{ route('groups.update', $group->id) }}">
                            ...
                            <select multiple class="form-control" id="selectTrainer" name="employee_id[]" required>
                                ...
                                @foreach ($trainers as $trainer)
                                    @if (in_array($trainer->id, $employees))
                                        <option value="{{ $trainer->id }}" selected>
                                            {{ $trainer->name . ' ' . $trainer->last_name }} </option>
                                    @else
                                        <option value="{{ $trainer->id }}"> {{ $trainer->name . ' ' . $trainer->last_name }}
                                        </option>
                        ...
                        <div class="row">
                            <div class="col">
                                <div class="form-group">
                                    ...
                                    <input type="date" class="form-control form-control-sm" id="InputStartDate" name="start_date"
                        ...
                        <div class="form-group">
                            <div class="form-check form-check-inline">
                                <input class="form-check-input" type="checkbox" id="inlineCheckbox1" name="frecuency[]"
                                    value="Lunes" {{ in_array('Lunes', $frecuency) ? 'checked' : '' }}>
                                <label class="form-check-label" for="inlineCheckbox1">Lunes</label>
                            </div>
                            <div class="form-check form-check-inline">
                                <input class="form-check-input" type="checkbox" id="inlineCheckbox2" name="frecuency[]"
                                    value="Martes" {{ in_array('Martes', $frecuency) ? 'checked' : '' }}>
                                <label class="form-check-label" for="inlineCheckbox2">Martes</label>
                            </div>
                            <div class="form-check form-check-inline">
                                <input class="form-check-input" type="checkbox" id="inlineCheckbox3" name="frecuency[]"
                                    value="Miércoles" {{ in_array('Miércoles', $frecuency) ? 'checked' : '' }}>
                                <label class="form-check-label" for="inlineCheckbox3">Miércoles</label>
                            </div>
                            <div class="form-check form-check-inline">
                                <input class="form-check-input" type="checkbox" id="inlineCheckbox4" name="frecuency[]"
                                    value="Jueves" {{ in_array('Jueves', $frecuency) ? 'checked' : '' }}>
                                <label class="form-check-label" for="inlineCheckbox4">Jueves</label>
                            </div>
                            <div class="form-check form-check-inline">
                                <input class="form-check-input" type="checkbox" id="inlineCheckbox5" name="frecuency[]"
                                    value="Viernes" {{ in_array('Viernes', $frecuency) ? 'checked' : '' }}>
                                <label class="form-check-label" for="inlineCheckbox5">Viernes</label>
                            </div>
                            <div class="form-check form-check-inline">
                                <input class="form-check-input" type="checkbox" id="inlineCheckbox6" name="frecuency[]"
                                    value="Sábado" {{ in_array('Sábado', $frecuency) ? 'checked' : '' }}>
                                <label class="form-check-label" for="inlineCheckbox6">Sábado</label>
                            </div>
                            <div class="form-check form-check-inline">
                                <input class="form-check-input" type="checkbox" id="inlineCheckbox7" name="frecuency[]"
                                    value="Domingo" {{ in_array('Domingo', $frecuency) ? 'checked' : '' }}>
                        ...
                        <div class="form-group student">
                            ...
                            @foreach ($customers as $customer)
                                @if (in_array($customer->id, $students))
                                    <div class="form-check">
                                        <input class="form-check-input" type="checkbox" value="{{ $customer->id }}"
                                            id="{{ $customer->id }}" name="customer_id[]" checked>
                                ...
                                @else
                                    <div class="form-check">
                                        <input class="form-check-input" type="checkbox" value="{{ $customer->id }}"
                                            id="{{ $customer->id }}" name="customer_id[]">
                                        <label class="form-check-label" for="{{ $customer->id }}"
                                            title="Es miembro: {{ $customer->member }}">
                                            {{ $customer->name . ' ' . $customer->last_name }}
                                        </label>
